...
function roiPctGrupo(greens: number, reds: number, picks: QuickPickRow[]): number | null {
  ...
  for (const p of picks) {
    if (p.resultado === "green") lucro += p.odd - 1;
    else if (p.resultado === "red") lucro -= 1;
  }
...
export function computePerformanceModel(picks: QuickPickRow[]): PerformanceModel {
  ...
  for (const p of encerradas) {
    if (p.resultado === "green") lucroAcumuladoUnidades += p.odd - 1;
    else if (p.resultado === "red") lucroAcumuladoUnidades -= 1;
  }
  ...
  const top = mercadosSorted.slice(0, TOP_MERCADOS);
  const rest = mercadosSorted.slice(TOP_MERCADOS);
  const porMercado: BreakdownRow[] = top.map(([key, a]) =>
    toBreakdownRow(
      key,
  ...
  return {
    ...
    oddMedia,
    apostasComResultado,
    melhorSequenciaGreen: maxG,
    piorSequenciaRed: maxR,
    porEsporte,
    porMercado,
